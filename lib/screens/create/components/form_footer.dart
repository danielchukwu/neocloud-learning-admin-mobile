...
class FormFooter extends StatelessWidget {
  ...
  TextButton _buildFormButton(BuildContext context) {
    return TextButton(
      style: ButtonStyle(
        ...
        backgroundColor:
            MaterialStatePropertyAll(_btnIsLoading ? Colors.blue[100] : kBlue),
        // padding: MaterialStatePropertyAll<EdgeInsets>(EdgeInsets.symmetric(horizontal: 20)),
      ),
      onPressed: _btnIsLoading ? null : () => _onSumbit(context),
      child: _btnIsLoading == false
          ...
          : buildCirularProgressIndicator(context),
    );
  }

  SizedBox buildCirularProgressIndicator(BuildContext context) {
    return SizedBox(
      width: 10,
      height: 10,
      child: CircularProgressIndicator(
          color: Theme.of(context).canvasColor.withOpacity(.7), strokeWidth: 2),
    );
  }
